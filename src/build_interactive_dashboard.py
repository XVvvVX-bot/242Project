  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    @media (max-width: 900px) {
      ...
      h2 { font-size: 32px; }
      .lead { font-size: 17px; }
      .metric-grid, .card-grid, .process, .takeaway-grid, .schema-row, .flow-item { grid-template-columns: 1fr; }
...
<body>
  <div class="deck-shell">
    ...
    <main>
      <section class="slide single-slide active">
        <div>
      ...
      <section class="slide content-slide">
        <div>
          ...
          <p class="lead">RevIN is a time-series normalization method for distribution shift: it lets the model learn the shape of recent demand separately from the current demand level.</p>
          <p class="speaker-note">This is useful in retail because demand can shift after holidays, events, promotions, stockout recovery, or gradual trend changes.</p>
        </div>
        <div class="formula-panel">
          <div class="formula">\[
            \mathbf{x}_{s,t} = \left[y_{s,t-L}, \ldots, y_{s,t-1}\right],
            \quad
            \mu_{s,t} = \frac{1}{L}\sum_{i=1}^{L} x_{s,t,i}
          \]</div>
          <div class="formula">\[
            \sigma_{s,t} =
            \sqrt{\frac{1}{L}\sum_{i=1}^{L}\left(x_{s,t,i}-\mu_{s,t}\right)^2 + \epsilon},
            \quad
            z_{s,t,i} = \frac{x_{s,t,i}-\mu_{s,t}}{\sigma_{s,t}}
          \]</div>
          <div class="formula">\[
            \hat{z}_{s,t}=f_{\theta}\left(\mathbf{z}_{s,t}, \mathbf{c}_{s,t}, s\right),
            \quad
            \hat{y}_{s,t} = \hat{z}_{s,t}\sigma_{s,t} + \mu_{s,t}
          \]</div>
          <div class="flow-list">
            <div class="flow-item"><strong>1</strong><span>Normalize each rolling input window using only past values in that window.</span></div>
            <div class="flow-item"><strong>2</strong><span>Train the MLP or Transformer to predict next-day demand in this local normalized space.</span></div>
            <div class="flow-item"><strong>3</strong><span>Reverse the transformation with the same window mean and standard deviation to return to unit sales.</span></div>
            <div class="flow-item"><strong>4</strong><span>No target leakage: the target day is not used when computing mu or sigma.</span></div>
          </div>
        </div>
      </section>
      ...
      <section class="slide content-slide">
        <div>
          <h2>RevIN experiment and tuning</h2>
          <p class="lead">We add RevIN versions of both neural models, then tune both model families under the same validation design.</p>
          <p class="speaker-note">The experiment asks whether local normalization improves the models we trained, not whether a prebuilt forecasting model can be reused.</p>
        </div>
        <div class="card-grid">
